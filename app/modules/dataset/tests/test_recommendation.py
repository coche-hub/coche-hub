...
from app.modules.dataset.models import CSVDataSet, DSMetaData, PublicationType, Author
...
def test_get_recommended_datasets_sorted_by_similarity(test_client):
    ...
    # Very different
    ds_different = create_test_dataset(
        user.id, "Different",
        pub_type=PublicationType.DATA_MANAGEMENT_PLAN,
        tags="java",
        authors_list=["Charlie"]
    )
...
def test_get_unsynchronized_dataset_with_recommendations(test_client):
    """Test get_unsynchronized_dataset route returns recommendations"""
    ...
    similar_ds = create_test_dataset(user.id, "Similar", tags="test")
